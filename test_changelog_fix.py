...
def test_change_tracking():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        changelog = {
            ...
            "changes_by_type": {
                "css_variable": {  # SINGULAR!
                    "added": [{"name": "--new-var"}],
                    "modified": [{"name": "--existing-var", "old_values": "original"}],
                    "removed": [],
                }
            },
        }
        ...
        assert (
            scroller["first_seen"] == "2026.0.4"
        ), f"first_seen should be 2026.0.4, got {scroller['first_seen']}"
        assert (
            scroller["last_seen"] == "2026.0.5-beta"
        ), f"last_seen should be 2026.0.5-beta, got {scroller['last_seen']}"
        assert (
            scroller["change_status"] == "unchanged"
        ), f"change_status should be unchanged, got {scroller['change_status']}"

        # Check --existing-var (modified)
        existing = results_by_name["--existing-var"]
        assert (
            existing["first_seen"] == "2026.0.3"
        ), f"first_seen should be 2026.0.3, got {existing['first_seen']}"
        assert (
            existing["last_seen"] == "2026.0.5-beta"
        ), f"last_seen should be 2026.0.5-beta, got {existing['last_seen']}"
        assert (
            existing["change_status"] == "modified"
        ), f"change_status should be modified, got {existing['change_status']}"
        assert (
            existing["changed_in_version"] == "2026.0.5-beta"
        ), f"changed_in_version should be 2026.0.5-beta, got {existing['changed_in_version']}"

        # Check --new-var (added)
        new_var = results_by_name["--new-var"]
        assert (
            new_var["first_seen"] == "2026.0.5-beta"
        ), f"first_seen should be 2026.0.5-beta, got {new_var['first_seen']}"
        assert (
            new_var["last_seen"] == "2026.0.5-beta"
        ), f"last_seen should be 2026.0.5-beta, got {new_var['last_seen']}"
        assert (
            new_var["change_status"] == "new"
        ), f"change_status should be new, got {new_var['change_status']}"
