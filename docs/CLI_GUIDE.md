...
- `--out` — Destination for modified bundles (required).
...
- Modified bundles land in `<output_dir>` suffixed with `_modified.bundle`.
...
2. Dry run the patch to confirm what would change:
   ...
   ```bash
   python -m src.cli.main patch skins/my_skin --out build/test --dry-run
   ```
...
4. Remove `--dry-run` to write patched bundles once you are satisfied:
   ...
   ```bash
   python -m src.cli.main patch skins/my_skin --out build/test --backup
   ```

5. Copy the `_modified.bundle` files into Football Manager's `data` override folder (per your modding setup).
...
- Patched bundles: `<out>/*_modified.bundle`
