...
def parse_element_at_offset(
    raw_data: bytes, offset: int, debug: bool = False
) -> Optional[UXMLElementBinary]:
    """
    ...
    try:
        ...
        if debug:
            ...
            print(
                f"  m_Id: {m_id}, m_Order: {m_order}, m_Parent: {m_parent}, m_Rule: {m_rule}"
            )
        ...
        def read_string_array(pos, array_name):
            ...
            for i in range(count):
                if pos + 4 > len(raw_data):
                    if debug:
                        print(
                            f"DEBUG: Not enough data to read {array_name}[{i}] length at {pos}"
                        )
                    return None, pos
                ...
                if pos + str_len + 1 > len(raw_data):
                    if debug:
                        print(
                            f"DEBUG: Not enough data to read {array_name}[{i}] string at {pos}"
                        )
                    return None, pos
                ...
                try:
                    ...
                    if debug:
                        print(
                            f"DEBUG: Failed to decode {array_name} string at pos {pos}: {e}"
                        )
                ...
                if pos % 4 == 0:
                    ...
                    if remainder != 0:
                        ...
                        if debug and i < count - 1:
                            print(
                                f"    added {padding} bytes padding to align to {pos}"
                            )
        ...
        if debug:
            print(
                f"  serialization fields: ({unknown_field_3}, {m_serialized_data}, {unknown_field_4}, {unknown_field_5}) at pos {pos-16}"
            )
        ...
        if remainder != 0:
            ...
            if debug:
                print(
                    f"    added {padding} bytes of padding after m_Type, now at pos {pos}"
                )
