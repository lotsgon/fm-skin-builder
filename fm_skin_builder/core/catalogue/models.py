...
class CatalogueMetadata(BaseModel):
    ...
    catalogue_version: int = Field(..., description="Catalogue rebuild version (e.g., 2)")
    fm_version: str = Field(..., description="FM game version (e.g., '2026.4.0')")
    schema_version: str = Field(default="1.0.0", description="Catalogue data format version")
...
class CSSValueDefinition(BaseModel):
    ...
    value_type: int = Field(..., description="Unity type: 3=dimension, 4=color, 8=string, 10=variable")
    index: int = Field(..., description="Index in strings/colors array")
    resolved_value: str = Field(..., description="Human-readable: '#1976d2' or 'var(--primary)' or '10px'")
...
class CSSClass(BaseModel):
    ...
    tags: List[str] = Field(default_factory=list, description="Auto-tags: ['button', 'primary', 'ui']")
...
class Sprite(BaseModel):
    ...
    has_vertex_data: bool = Field(
        default=False,
        description="True = vector sprite (custom mesh data)"
    )
